...
//			  , "" + (System.currentTimeMillis() - millis));
    ...
    public static String getDataColumn(final Context context, Uri uri, String selection,
        ...
        try {
			...
			final String pre = path.substring(0, indexOf);
			final String storage0 = Environment.getExternalStorageDirectory() .toString();
			if (pre.endsWith("primary")) {
				return storage0 + "/" + path.substring(indexOf + 1);
			} else {
				String extCard = pre.substring(pre.lastIndexOf("/"), indexOf);
				return "/storage" + extCard + "/" + path.substring(indexOf + 1);
			}
